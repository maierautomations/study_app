...
import { parseBody, flashcardGenerateSchema } from "@/lib/validations";
...
export async function POST(request: NextRequest) {
  ...
  // Rate limit
  const rl = rateLimit(`${user.id}:flashcard-generate`, AI_RATE_LIMIT.maxRequests, AI_RATE_LIMIT.windowMs);
  if (!rl.success) {
  ...
  const parsed = parseBody(flashcardGenerateSchema, body);
  if (!parsed.success) {
