...
- **Database types**: `src/types/database.ts` — manually maintained typed schema matching `supabase/migrations/00001_initial_schema.sql`
...
- RAG Chat: `POST /api/chat` uses `streamText()` with embedding-based retrieval; client uses `useChat` from `@ai-sdk/react`
